...
template <class T>
LinkedList<T> LinkedList<T>::finder(T example)
{
    return Algorithm<LinkedListIterator<T>, T>::findAllWithParameters(begin(), end(), example);
}
...
template <typename T>
Node<T> *LinkedListIterator<T>::getNode() const
{
    return current;
}