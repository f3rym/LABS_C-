...
void MonoBlock::sortList(std::vector<ComputingMachine *> &list)
{
    ...
    std::vector<MonoBlock *> monoList;
    for (auto *ptr : list)
    {
        if (auto *mono = dynamic_cast<MonoBlock *>(ptr))
        {
    ...
    {
    case 1:
        std::sort(monoList.begin(), monoList.end(),
                  [](MonoBlock *a, MonoBlock *b)
                  {
                      return a->sizeDisplay > b->sizeDisplay;
                  });
    ...
    case 2:
        std::sort(monoList.begin(), monoList.end(),
                  [](MonoBlock *a, MonoBlock *b)
                  {
                      return a->touchScreen > b->touchScreen;
                  });
    ...
    {
        if (dynamic_cast<MonoBlock *>(list[i]))
        {
            list[i] = monoList[monoIndex++];
        }
    }
}
